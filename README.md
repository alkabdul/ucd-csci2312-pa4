...
### Synopsis
    ...
    The main goal for this progect  to give a fertile setting for dynamic (run-time) polymorphism of user types. You will create a class hierarchy with a rich tapestry of polymorphic behavior. You have to write quite a few more files , though most are small: `Exceptions.cpp`, `Piece.cpp`, `Agent.cpp`, `Simple.cpp`, `Strategic.cpp`, `Resource.cpp`, `Food.cpp`, `Advantage.cpp`, `Game.cpp`, `DefaultAgentStrategy.cpp`, and `AggressiveAgentStrategy.cpp`. See the [Detailed Instructions]) at the bottom of this file.

This program  is in the test-driven-development (TDD) style . It has 153 tests that your implementation should pass. Because the different elements of the `Game` are quite intertwined, it is recommended that you follow roughly this order of implementation and testing:
...
Familiarize yourself with and start following [coding style guidelines](http://courses.cms.caltech.edu/cs11/material/cpp/donnie/cppstyle.html). There are others on the Web. Pick one and be consistent.### References

Operator overloading [guidelines](http://courses.cms.caltech.edu/cs11/material/cpp/donnie/cpp-ops.html). For the implementation of `virtual Piece &operator*(Piece &other)`,
...
This program creates a simple board turn-based Game where `Piece`-s move around and interact. This is how a short run of the Game looks like in print:
...
`Piece`-s are of different type and have polymorphic (i.e. different) behavior. 
...
For the implementation of the `Piece::operator*()`, take a look at the examples file
